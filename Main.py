...
def evaluatePredictions(tokenToPrediction):
    ...
    #for key in correct:
    #    row += [key, round(float(correct[key])/float(correct[key]+wrong[key])*100,2), float(correct[key]+wrong[key])]
...
if __name__ == '__main__':
    ...
    step2 = False
    if step2:
        squaredWeight = False #This parameter raises the weight associated by Google via weight=weight*weight
